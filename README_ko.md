...
- 확장자별 색상 배지
- 폴더 우선, 이름순 정렬
- **파일 타임스탬프** — 수정시각 초 단위 표시, 생성일은 수정일과 다를 때만 표시
- **반응형 사이드바** — 300px 이하로 좁히면 용량·생성일 숨기고 배지+수정일만 표시

### 전체 텍스트 검색 & 필터

- 파일명 **및** 파일 내용 동시 검색 (AND/OR 연산자 지원)
- **매치 카운트 배지** — 파일별 총 매치 개수 표시 (파일명 + 본문)
- **매치순 정렬** — 검색 모드에서 Matches 소트 버튼 자동 표시
- **날짜 필터** — Modified/Created 각각 from~to 범위 캘린더 피커
- **매치 수 필터** — min~max 범위로 검색 결과 필터링
- **교차 필터** — 날짜 필터와 매치 필터 동시 적용

### 마크다운 렌더링

외부 라이브러리 없는 커스텀 마크다운 파서:

- 제목 (h1~h6) 크기별 색상 구분 + 자동 앵커 ID 생성
- **문서 내 섹션 링크** — `[텍스트](#헤딩-앵커)` 클릭 시 해당 섹션으로 부드럽게 스크롤 (페이지 리로드 없음)
- **굵게**, *기울임*, ~~취소선~~, `인라인 코드`
